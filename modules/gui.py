...
class SpinOSGUI:
    ...
    def __init__(self, master, wwd, width, height):
        ...
        s.map("TEntry", foreground=[('disabled', 'gray'), ('!disabled', 'black')])
        s.map("TLabel", foreground=[('disabled', 'gray'), ('!disabled', 'black')])
        s.map("TButton", foreground=[('disabled', 'gray'), ('!disabled', 'black')])
        s.map("TRadiobutton", foreground=[('disabled', 'gray'), ('!disabled', 'black')])
        ...
        firstlabel = ttk.Label(filesframe, text='DATA', font=('', cst.TITLESIZE, 'underline'),
                               state=tk.ACTIVE)
        firstlabel.grid(columnspan=5, sticky=tk.N)

        # define inclusion variables
        self.load_rv1 = tk.BooleanVar()
        self.load_rv2 = tk.BooleanVar()
        self.load_as = tk.BooleanVar()

        # assign to check buttons
        rv1check = ttk.Checkbutton(filesframe, variable=self.load_rv1, command=self.toggle_rv1)
        rv2check = ttk.Checkbutton(filesframe, variable=self.load_rv2, command=self.toggle_rv2)
        ascheck = ttk.Checkbutton(filesframe, variable=self.load_as, command=self.toggle_as)
        ...
        ttk.Label(filesframe, text='Working directory').grid(row=1, column=1, sticky=tk.E)
        self.rv1_label = ttk.Label(filesframe, text='Primary RV file', state=tk.DISABLED)
        self.rv1_label.grid(row=2, column=1, sticky=tk.E)
        self.rv2_label = ttk.Label(filesframe, text='Secondary RV file', state=tk.DISABLED)
        self.rv2_label.grid(row=3, column=1, sticky=tk.E)
        self.as_label = ttk.Label(filesframe, text='Astrometric data file', state=tk.DISABLED)
        ...
        self.seppa_but = ttk.Radiobutton(filesframe, text='Sep/PA', variable=self.seppa, value=True,
                                         state=tk.DISABLED)
        self.seppa_but.grid(row=4, column=3)
        self.en_but = ttk.Radiobutton(filesframe, text='E/N', variable=self.seppa, value=False,
                                      state=tk.DISABLED)
        self.en_but.grid(row=4, column=4, sticky=tk.W)

        ttk.Button(filesframe, text='Load Data', command=self.datamanager.loaddataintoSets, ).grid(
            row=5, columnspan=5, pady=10)
        ...
        self.seppa_but_dtmgr = ttk.Radiobutton(self.as_tab, text='Sep/PA', variable=self.seppa,
                                               value=True)
        self.seppa_but_dtmgr.pack(side=tk.LEFT, expand=1)
        self.en_but_dtmgr = ttk.Radiobutton(self.as_tab, text='E/N', variable=self.seppa,
                                            value=False)
        self.en_but_dtmgr.pack(side=tk.LEFT, expand=1)
        ttk.Button(self.as_tab, text='Add Dataset', command=lambda: self.datamanager.emptyasDataSet(
            seppa=self.seppa_dtmgr.get())).pack(side=tk.LEFT, expand=1)
        ...
        # region Guess Frame
        self.system: Optional[bsys.BinarySystem] = None
        ...
        ttk.Label(guess_frame, text='SYSTEM PARAMETERS',
                  font=('', cst.TITLESIZE, 'underline')).grid(columnspan=columns)

        ttk.Label(guess_frame, text='Guess file').grid(row=1, column=labelcolumn, sticky=tk.E)
        ...
        ttk.Label(guess_frame, text='Guesses').grid(row=titlesrow, column=entrycolumn)
        ttk.Label(guess_frame, text='Vary?').grid(row=titlesrow, column=varycheckcolumn)
        ttk.Label(guess_frame, text='Transfer').grid(row=titlesrow, column=transfercolumn)
        ttk.Label(guess_frame, text='Result').grid(row=titlesrow, column=minresultcolumn)
        ttk.Label(guess_frame, text='Error').grid(row=titlesrow, column=errorcolumn)

        self.lock_gs = tk.BooleanVar(value=False)
        ...
        self.lock_gs_button = ttk.Button(guess_frame, width=1, image=self.locked_image,
                                         command=self.toggle_lock)
        self.lock_gs_button.grid(row=paramgridrow + 10)

        self.q_mode = tk.BooleanVar(value=False)
        self.lock_q_button = ttk.Button(guess_frame, width=1, text='q', command=self.toggle_q)
        ...
        self.param_label_list = [ttk.Label(guess_frame, textvariable=self.param_var_list[i]) for i
                                 in rparams]
        ...
        # define check buttons for vary states
        self.vary_button_list = [ttk.Checkbutton(guess_frame, variable=self.vary_var_list[i]) for i
                                 in rparams]

        # put the check buttons in a nice grid
        for i in rparams:
        ...
        # list comprehension
        self.transfer_button_list = [ttk.Button(guess_frame, text='<-', width=4,
        ...
        self.min_label_list = [
            ttk.Label(guess_frame, textvariable=self.mininimzed_var_list[i], width=8) for i in
            rparams]
        ...
        self.error_label_list = [
            ttk.Label(guess_frame, textvariable=self.error_var_list[i], width=8) for i in rparams]
        for i in rparams:
            self.error_label_list[i].grid(row=paramgridrow + i, column=errorcolumn)

        # define the buttons in this frame
        ttk.Button(guess_frame, text='Load guesses', command=self.load_guesses).grid(row=buttonrow,
                                                                                     column=labelcolumn)
        ttk.Button(guess_frame, text='Save guesses', command=self.save_guesses).grid(row=buttonrow,
            column=entrycolumn)
        ttk.Button(guess_frame, text='Save parameters', command=self.save_params).grid(
        ...
        ttk.Label(infer_frame, text='From k1/k2', font=('', 13, 'underline')).grid(row=1,
                                                                                   columnspan=2)
        ...
        ttk.Label(infer_frame, textvariable=self.mprimary).grid(row=3, column=1)
        ttk.Label(infer_frame, textvariable=self.msecondary).grid(row=4, column=1)
        ttk.Label(infer_frame, textvariable=self.semimajork1k2).grid(row=2, column=1)
        ttk.Label(infer_frame, textvariable=self.totalmass).grid(row=5, column=1)
        ttk.Separator(infer_frame).grid(column=2, row=2, rowspan=5, sticky=tk.NS)
        ttk.Label(infer_frame, text='From d/M_tot:', font=('', 13, 'underline')).grid(row=1,
                                                                                      column=3,
                                                                                      columnspan=2)
        ...
        self.hjd_calc_east_pa_var = tk.StringVar(value='East (mas)')
        ttk.Label(self.hjd_entry_frame, textvariable=self.hjd_calc_north_sep_var).grid(row=0,
                                                                                       column=5)
        ttk.Label(self.hjd_entry_frame, textvariable=self.hjd_calc_east_pa_var).grid(row=0,
                                                                                     column=6)

        self.number_of_lines = 0
        self.hjd_calc_entries = []
        self.rv1_results = []
        self.rv2_results = []
        ...
        ttk.Label(methodframe, text='MINIMIZATION', font=('', cst.TITLESIZE, 'underline')).grid(
            columnspan=4)
        ttk.Label(methodframe, text='Method:').grid(sticky=tk.E)
        ttk.Radiobutton(methodframe, text='Levenberg-Marquardt', variable=self.method,
        ...
        self.as_weight_button = ttk.Checkbutton(otherminframe, variable=self.do_custom_weight,
        ...
        self.weight_slider = tk.Scale(otherminframe, variable=self.custom_as_weight, from_=0, to=1,
                                      resolution=0.01, orient=tk.HORIZONTAL, fg=cst.FONTCOLOR,
                                      state=tk.DISABLED, length=180)
        self.weight_slider.grid(row=3, column=2, columnspan=2, sticky=tk.W)

        ttk.Button(otherminframe, text='Minimize!', command=self.minimize).grid(row=4, columnspan=4)
        ttk.Label(otherminframe, text='Results', font=('', cst.TITLESIZE, 'underline')).grid(row=5,
                                                                                             columnspan=4)
        ttk.Label(otherminframe, text='Red. Chi Sqrd =').grid(row=6, sticky=tk.E)
        ttk.Label(otherminframe, text='Deg. of frdm =').grid(row=7, sticky=tk.E)
        ttk.Label(otherminframe, textvariable=self.redchisq).grid(row=6, column=1, sticky=tk.W)
        ttk.Label(otherminframe, textvariable=self.dof).grid(row=7, column=1, sticky=tk.W)
        ttk.Label(otherminframe, text='RMS Primary (km/s) =').grid(row=6, column=2, sticky=tk.E)
        ttk.Label(otherminframe, text='RMS Secondary (km/s) =').grid(row=7, column=2, sticky=tk.E)
        ttk.Label(otherminframe, text='RMS Rel. Orbit (mas) =').grid(row=8, column=2, sticky=tk.E)
        ttk.Label(otherminframe, textvariable=self.rms_rv1).grid(row=6, column=3, sticky=tk.W)
        ttk.Label(otherminframe, textvariable=self.rms_rv2).grid(row=7, column=3, sticky=tk.W)
        ttk.Label(otherminframe, textvariable=self.rms_as).grid(row=8, column=3, sticky=tk.W)
        ...
        ttk.Label(plt_frame, text='PLOT CONTROLS', font=('', cst.TITLESIZE, 'underline')).grid(
            columnspan=6)
        # UI elements
        self.phase_label = ttk.Label(plt_frame, text='phase =', state=tk.DISABLED)
        ...
        self.phase_button = ttk.Checkbutton(plt_frame, var=self.plotter.do_phasedot,
                                            command=self.toggle_dot, state=tk.DISABLED)
        self.phase_button.grid(row=1)

        self.plot_rv1data_label = ttk.Label(plt_frame, text='Primary RV data', state=tk.DISABLED)
        self.plot_rv1data_label.grid(row=2, column=1)
        self.plot_rv1data_button = ttk.Checkbutton(plt_frame, var=self.plotter.do_datarv1,
                                                   state=tk.DISABLED)
        self.plot_rv1data_button.grid(row=2)

        self.plot_rv2data_label = ttk.Label(plt_frame, text='Secondary RV data', state=tk.DISABLED)
        self.plot_rv2data_label.grid(row=3, column=1)
        self.plot_rv2data_button = ttk.Checkbutton(plt_frame, var=self.plotter.do_datarv2,
                                                   state=tk.DISABLED)
        self.plot_rv2data_button.grid(row=3)

        self.plot_asdata_label = ttk.Label(plt_frame, text='Astrometric data', state=tk.DISABLED)
        self.plot_asdata_label.grid(row=4, column=1)
        self.plot_asdata_button = ttk.Checkbutton(plt_frame, var=self.plotter.do_dataas,
                                                  state=tk.DISABLED)
        self.plot_asdata_button.grid(row=4)

        self.plot_rv1model_label = ttk.Label(plt_frame, text='Primary RV model', state=tk.DISABLED)
        self.plot_rv1model_label.grid(row=2, column=3)
        self.plot_rv1model_button = ttk.Checkbutton(plt_frame, variable=self.plotter.do_modelrv1,
                                                    state=tk.DISABLED)
        self.plot_rv1model_button.grid(row=2, column=2)

        self.plot_gamma1_label = ttk.Label(plt_frame, text=r'Gamma 1', state=tk.DISABLED)
        self.plot_gamma1_label.grid(row=3, column=3)
        self.plot_gamma1_button = ttk.Checkbutton(plt_frame, variable=self.plotter.do_modelgamma1,
                                                  state=tk.DISABLED)
        self.plot_gamma1_button.grid(row=3, column=2)

        self.plot_rv2model_label = ttk.Label(plt_frame, text='Secondary RV model',
                                             state=tk.DISABLED)
        self.plot_rv2model_label.grid(row=4, column=3)
        self.plot_rv2model_button = ttk.Checkbutton(plt_frame, variable=self.plotter.do_modelrv2,
                                                    state=tk.DISABLED)
        self.plot_rv2model_button.grid(row=4, column=2)

        self.plot_gamma2_label = ttk.Label(plt_frame, text='Gamma 2', state=tk.DISABLED)
        self.plot_gamma2_label.grid(row=5, column=3)
        self.plot_gamma2_button = ttk.Checkbutton(plt_frame, variable=self.plotter.do_modelgamma2,
                                                  state=tk.DISABLED)
        self.plot_gamma2_button.grid(row=5, column=2)

        self.plot_asmodel_label = ttk.Label(plt_frame, text='Model Orbit', state=tk.DISABLED)
        self.plot_asmodel_label.grid(row=2, column=5)
        self.plot_asmodel_button = ttk.Checkbutton(plt_frame, variable=self.plotter.do_modelas,
                                                   state=tk.DISABLED)
        self.plot_asmodel_button.grid(row=2, column=4)

        self.plot_nodeline_label = ttk.Label(plt_frame, text='Line of nodes', state=tk.DISABLED)
        self.plot_nodeline_label.grid(row=3, column=5)
        self.plot_nodeline_button = ttk.Checkbutton(plt_frame, variable=self.plotter.do_nodeline,
                                                    state=tk.DISABLED)
        self.plot_nodeline_button.grid(row=3, column=4)

        self.plot_semimajor_label = ttk.Label(plt_frame, text='Semi-major axis', state=tk.DISABLED)
        self.plot_semimajor_label.grid(row=4, column=5)
        self.plot_semimajor_button = ttk.Checkbutton(plt_frame, variable=self.plotter.do_semimajor,
                                                     state=tk.DISABLED)
        self.plot_semimajor_button.grid(row=4, column=4)

        self.plot_peri_label = ttk.Label(plt_frame, text='Periastron', state=tk.DISABLED)
        self.plot_peri_label.grid(row=5, column=5)
        self.plot_peri_button = ttk.Checkbutton(plt_frame, variable=self.plotter.do_peri,
                                                state=tk.DISABLED)
        self.plot_peri_button.grid(row=5, column=4)

        self.as_dist_label = ttk.Label(plt_frame, text='Astrometric errors', state=tk.DISABLED)
        self.as_dist_label.grid(row=6, column=5)
        self.as_dist_button = ttk.Checkbutton(plt_frame, variable=self.plotter.do_as_dist,
                                              state=tk.DISABLED)
        self.as_dist_button.grid(row=6, column=4)

        legend_button = ttk.Checkbutton(plt_frame, variable=self.plotter.do_legend)
        legend_button.grid(row=6)
        ttk.Label(plt_frame, text='Legend').grid(row=6, column=1)

        self.pphase_but = ttk.Radiobutton(plt_frame, text='phase', command=self.toggle_phase_time,
                                          variable=self.plotter.plot_vs_phase, value=True,
                                          state=tk.DISABLED)
        self.ptime_but = ttk.Radiobutton(plt_frame, text='time', command=self.toggle_phase_time,
        ...
        self.modelwidgets = {self.plot_asmodel_label, self.plot_asmodel_button,
                             ...
                             self.plot_rv1model_label, self.plot_rv2model_label,
                             self.plot_semimajor_button, self.plot_semimajor_label,
                             self.plot_nodeline_button, self.plot_nodeline_label,
                             self.plot_peri_label, self.plot_peri_button, self.as_dist_button,
                             self.as_dist_label, self.pphase_but, self.ptime_but,
                             self.plot_gamma1_button, self.plot_gamma1_label,
                             self.plot_gamma2_button, self.plot_gamma2_label}
        plt_frame.pack()

        ttk.Button(plt_frame_top, text='Refresh Plots', command=self.update, ).pack()

        settings_frame = ttk.Frame(plt_frame_top)
        entrycol = 1
        ttk.Label(settings_frame, text='PLOT SETTINGS', font=('', cst.TITLESIZE, 'underline')).grid(
            columnspan=3)

        ttk.Label(settings_frame, text='Axis label size').grid(row=1, sticky=tk.E)
        ttk.Entry(settings_frame, textvariable=self.plotter.axeslabelsize, width=15).grid(row=1,
                                                                                          column=entrycol,
                                                                                          columnspan=2)

        ttk.Label(settings_frame, text='Tick label size').grid(row=2, sticky=tk.E)
        ttk.Entry(settings_frame, textvariable=self.plotter.ticklabelsize, width=15).grid(row=2,
                                                                                          column=entrycol,
                                                                                          columnspan=2)

        ttk.Label(settings_frame, text='Grid visibility').grid(row=3, sticky=tk.E)

        self.grid_button = ttk.Checkbutton(settings_frame, variable=self.plotter.do_grids)
        self.grid_button.grid(row=3, column=1)

        ttk.Label(settings_frame, text='Axis limits').grid(row=4)
        ttk.Radiobutton(settings_frame, text='Auto', variable=self.plotter.limcontrol, value=True,
                        command=self.togglelimcontrol).grid(row=4, column=1)
        ttk.Radiobutton(settings_frame, text='Manual', variable=self.plotter.limcontrol,
                        value=False, command=self.togglelimcontrol).grid(row=4, column=2)

        limitrow = 5
        self.limit_labels = []
        for i in range(8):
            self.limit_labels.append(
                ttk.Label(settings_frame, text=cst.LIM_STRINGS[i], state=tk.DISABLED))
        ...
        for i in range(8):
            self.limit_entries.append(
                ttk.Entry(settings_frame, textvariable=self.plotter.limits[i], state=tk.DISABLED))
        for i in range(8):
            self.limit_entries[i].grid(row=limitrow + i, column=entrycol, columnspan=2)

        settings_frame.pack(pady=10)

        ttk.Button(plt_frame_top, text='Refresh Plots', command=self.plotter.update_plots).pack(
            pady=10)
        ttk.Button(plt_frame_top, text='New plot windows', command=self.plotter.init_plots).pack(
            pady=10)
    ...
    def toggle_q(self):
        ...
        if self.q_mode.get():
            ...
            if float(self.guess_var_list[8].get()) != 0:
                self.guess_var_list[8].set(str(np.round(
    ...
    def load_guesses(self):
        ...
        try:
            ...
            print('your guessfile seems to have badly formatted data or '
                  'something...')
            print(e)
            self.guess_dict = None
            return
        try:
            for i in range(len(cst.PARAM_LIST)):
                ...
                self.vary_var_list[i].set(bool(self.guess_dict[cst.PARAM_LIST[i]][1]))
        ...
        if self.guess_dict is not None and len(data_dict) > 0:
            ...
            try:
                if self.do_custom_weight.get():
                    ...
                    w = None
                self.minresult, rms_rv1, rms_rv2, rms_as = spm.LMminimizer(self.guess_dict,
                                                                           data_dict,
                                                                           self.method.get(),
                                                                           self.hops.get(),
                                                                           self.steps.get(),
                ...
                # fill in the entries
                self.mininimzed_var_list[0].set(str(np.round(pars['p'].value, 3)))
                if pars['p'].vary:
                    self.error_var_list[0].set(
                        str(np.round(0 if pars['p'].stderr is None else pars['p'].stderr, 3)))
                self.mininimzed_var_list[1].set(str(np.round(pars['e'].value, 3)))
                if pars['e'].vary:
                    self.error_var_list[1].set(
                        str(np.round(0 if pars['e'].stderr is None else pars['e'].stderr, 3)))
                self.mininimzed_var_list[2].set(str(np.round(pars['i'].value % 360, 3)))
                if pars['i'].vary:
                    self.error_var_list[2].set(
                        str(np.round(0 if pars['i'].stderr is None else pars['i'].stderr, 3)))
                self.mininimzed_var_list[3].set(str(np.round(pars['omega'].value % 360, 3)))
                if pars['omega'].vary:
                    self.error_var_list[3].set(
                        str(np.round(0 if pars['omega'].stderr is None else pars['omega'].stderr,
                                     3)))
                self.mininimzed_var_list[4].set(str(np.round(pars['Omega'].value % 360, 3)))
                if pars['Omega'].vary:
                    self.error_var_list[4].set(
                        str(np.round(0 if pars['Omega'].stderr is None else pars['Omega'].stderr,
                                     3)))
                self.mininimzed_var_list[5].set(str(np.round(pars['t0'].value, 3)))
                if pars['t0'].vary:
                    self.error_var_list[5].set(
                        str(np.round(0 if pars['t0'].stderr is None else pars['t0'].stderr, 3)))
                self.mininimzed_var_list[6].set(str(np.round(pars['d'].value, 3)))
                if pars['d'].vary:
                    self.error_var_list[6].set(
                        str(np.round(0 if pars['d'].stderr is None else pars['d'].stderr, 3)))
                self.mininimzed_var_list[7].set(str(np.round(pars['k1'].value, 3)))
                if pars['k1'].vary:
                    self.error_var_list[7].set(
                        str(np.round(0 if pars['k1'].stderr is None else pars['k1'].stderr, 3)))
                if self.q_mode.get():
                    self.mininimzed_var_list[8].set(str(np.round(pars['q'].value, 3)))
                else:
                    self.mininimzed_var_list[8].set(str(np.round(pars['k2'].value, 3)))
                    if pars['k2'].vary:
                        self.error_var_list[8].set(
                            str(np.round(0 if pars['k2'].stderr is None else pars['k2'].stderr, 3)))
                self.mininimzed_var_list[9].set(str(np.round(pars['gamma1'].value, 3)))
                if pars['gamma1'].vary:
                    self.error_var_list[9].set(
                        str(np.round(0 if pars['gamma1'].stderr is None else pars['gamma1'].stderr,
                                     3)))
                self.mininimzed_var_list[10].set(str(np.round(pars['gamma2'].value, 3)))
                if pars['gamma2'].vary:
                    self.error_var_list[10].set(
                        str(np.round(0 if pars['gamma2'].stderr is None else pars['gamma2'].stderr,
                                     3)))

                self.mininimzed_var_list[11].set(str(np.round(pars['mt'].value, 3)))
                if pars['mt'].vary:
                    self.error_var_list[11].set(
                        str(np.round(0 if pars['mt'].stderr is None else pars['mt'].stderr, 3)))

                self.redchisq.set(float(np.round(self.minresult.redchi, 4)))
                self.dof.set(self.minresult.nfree)
                self.rms_rv1.set(float(np.round(rms_rv1, 4)))
                self.rms_rv2.set(float(np.round(rms_rv2, 4)))
                self.rms_as.set(float(np.round(rms_as, 4)))
                self.minimization_run_number += 1
            except ValueError as e:
                print(e)
    ...
    def set_inferred_params(self):
        ...
        self.totalmass.set(str(np.round(self.system.total_mass(), 2)))
        self.semimajork1k2.set(str(np.round(self.system.semimajor_axis_from_RV(), 2)))
        self.semimajord.set(str(np.round(self.system.semimajor_axis_from_distance(), 2)))

    def set_hjd_calc_labels(self):
        if self.hjd_calc_in_north_east.get():
            self.hjd_calc_north_sep_var.set('North (mas)')
            self.hjd_calc_east_pa_var.set('East (mas)')
        else:
            self.hjd_calc_north_sep_var.set('Sep (mas)')
            self.hjd_calc_east_pa_var.set('PA (deg)')
    ...
    def add_hjd_calc_line(self):
        ...
        self.rv1_results.append(tk.StringVar())
        self.rv2_results.append(tk.StringVar())
        self.north_sep_results.append(tk.StringVar())
        self.east_pa_results.append(tk.StringVar())
        ttk.Label(self.hjd_entry_frame, textvariable=self.rv1_results[-1]).grid(
            row=self.number_of_lines, column=3)
        ttk.Label(self.hjd_entry_frame, textvariable=self.rv2_results[-1]).grid(
            row=self.number_of_lines, column=4)
        ttk.Label(self.hjd_entry_frame, textvariable=self.north_sep_results[-1]).grid(
            row=self.number_of_lines, column=5)
        ttk.Label(self.hjd_entry_frame, textvariable=self.east_pa_results[-1]).grid(
            row=self.number_of_lines, column=6)

    def calculate_hjd_states(self):
        for i in range(len(self.hjd_calc_entries)):
            time = float(self.hjd_calc_entries[i].get())
            self.rv1_results[i].set(str(np.round(self.system.primary.radial_velocity_of_hjd(
                time), 3)))
            self.rv2_results[i].set(str(np.round(self.system.secondary.radial_velocity_of_hjd(
                time), 3)))
            if self.hjd_calc_in_north_east.get():
                self.north_sep_results[i].set(str(np.round(self.system.relative.north_of_hjd(
                    time), 3)))
                self.east_pa_results[i].set(str(np.round(self.system.relative.east_of_hjd(time),
                                                         3)))
            else:
                self.north_sep_results[i].set(str(np.round(self.system.relative.separation_of_hjd(
                    time), 3)))
                self.east_pa_results[i].set(str(np.round(self.system.relative.position_angle_of_hjd(
                    time), 3)))
    ...
    def save_params(self):
        ...
        with open(wd + out + '{}.txt'.format(self.minimization_run_number), 'w') as f:
            if self.minresult is not None:
                ...
                # f.write('dof = {} \n'.format(self.dof.get()))
        if self.didmcmc:
            np.savetxt(wd + out + '{}_flatchain.txt'.format(self.minimization_run_number),
